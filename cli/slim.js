...
if (cmd === "profile") {
  ...
  console.log(table(
    ...
    [
      ...
      ["Trial limit",  s.paid ? "∞" : (!s.checked ? c.yellow("unknown (server unreachable)") : s.usd_limit ? "$" + Number(s.usd_limit).toFixed(2) : "no limit")],
      ["Used this month", s.paid ? "—" : (!s.checked ? c.yellow("unknown") : s.used_usd == null ? "—" : "$" + Number(s.used_usd).toFixed(2))],
      ["Trial left",   s.paid ? "∞" : (!s.checked ? c.yellow("unknown") : s.remaining_usd == null ? "—" : "$" + Number(s.remaining_usd).toFixed(2))],
      ["Usage reset",  s.paid ? "—" : (!s.checked ? c.yellow("unknown") : s.reset_at ? c.cyan(s.reset_at) + c.gray(" (1st of month)") : "—")],
      ["Version",      versionVal],
    ],
    { align: ["left", "left"] },
  ));
